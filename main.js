...
*/
const typeSale = transactions.filter(transaction => transaction.type == 'sale');
const numSales = typeSale.length;
...
// */
const typePurchase = transactions.filter(transaction => transaction.type == 'purchase');
const numPurchases = typePurchase.length;
//
...
// */
const cashSales = typeSale.filter(cashPay => cashPay.paymentMethod == 'cash');
const numCashSales = cashSales.length;
//
...
// */
const credPurchases = typePurchase.filter(credPay => credPay.paymentMethod == 'credit');
const numCreditPurchases = credPurchases.length;
//
...
// */
const vendorObjects = transactions.filter(transaction => transaction.vendor);
const allVendors = vendorObjects.map(item => item.vendor);
...
const uniqueCustomers = typeSale.map(name => name.customer).filter((x, i, a) => a.indexOf(x) === i);
...
const bigSpenders = typeSale.filter(instance => instance.items.length >= 5).map(obj => (({name: obj.customer, numItems: obj.items.length})));
...
// */
const test = typeSale[0].items;
const sumFirstSale = reductor(test);

function reductor(array) {
  let sum = 0;
...
const purchaseItemMap = typePurchase.map(instance => instance.items);

function superSum (arrayOfArrays) {
  toTotal = [];
  for (var i = 0; i < arrayOfArrays.length; i++) {
    toTotal.push(reductor(arrayOfArrays[i]));
  }
  final = toTotal.reduce((total, value) => total + value, 0);
  return final;
}

const sumPurchases = superSum(purchaseItemMap);
//-33.2
...
// */
const saleItemMap = typeSale.map(instance => instance.items);
const sumSales = superSum(saleItemMap);
...
const saleArray = [];
const saleList = saleItemMap.forEach(function(element){
  saleArray.push(element.length);
});
...
// */
const mostItems = saleArray.reduce((a,b) => a > b ? a : b, '');
...
// */
const purchaseList = purchaseItemMap.sort(function(a,b) {return a.length - b.length;});
// console.log(purchaseList);
const sumOfSmallestPurchase = reductor(purchaseList[0]);
